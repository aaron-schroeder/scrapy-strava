...
class SimpleStravaRateLimitMiddleware:
    ACTIVE = 'active'

    def process_request(self, request, spider):
        # If the rate limit status is unknown, assume it is ok.
        rate_limit_status = getattr(spider, 'rate_limit_status', None)
        if (
    ...
    def process_response(self, request, response, spider):
        if response.status == 429:
            ...
            spider.rate_limit_status = self.ACTIVE

            # CloseSpider('rate_limit')

            # return
        
        # TODO: Move this to an appropriate method. It has to do with other
        # aspects of handling Strava API responses, not rate limits.
...
class RedisStravaRateLimitMiddleware:
    ACTIVE = 'active'
